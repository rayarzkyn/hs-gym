...
export default function AttendanceTracker({ data = [], detailed = false }: AttendanceTrackerProps) {
  ...
  const todayAttendees = attendance.filter(a => {
    ...
    const recordDate = new Date(a.checkInTime).toISOString().split('T')[0];
    return recordDate === today;
  });

  // Current attendees (yang belum check-out)
  const currentAttendees = todayAttendees.filter(a => !a.checkOutTime);
            ...
            {currentAttendees.map((attendee) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <div className="font-semibold text-green-800">{attendee.userName}</div>
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${
                    attendee.checkOutTime ? 'bg-gray-400' : 'bg-green-500 animate-pulse'
                  }`}></div>
                  <div>
                    <div className="font-medium text-gray-800">{attendee.userName}</div>
                    <div className="text-sm text-gray-600">
                      {attendee.type === 'member' ? '🎯 Member' : '👤 Non-Member'}
                      {attendee.facility && ` • ${attendee.facility}`}
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  ...
                  {attendee.checkOutTime && (
                    <div className="text-xs text-gray-500">
                      Out: {formatTime(attendee.checkOutTime)}
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-4 text-center">
        ...
        <div className="bg-orange-50 rounded-lg p-3">
          <div className="text-lg font-bold text-orange-600">
            {todayAttendees.filter(a => a.type === 'non_member' || a.type === 'non-member').length}
          </div>
